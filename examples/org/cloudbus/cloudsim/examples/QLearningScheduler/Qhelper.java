...
//		List<PowerHost> hostList = createHostList(Constants.NUMBER_OF_HOSTS);
...
//		try {
//			datacenter = new QlearningPowerDataCenter(name, characteristics, new QPowerVmAllocationPolicy(hostList),
...
//	}
	...
	public static List<Vm> createVmList(int brokerId, int vmsNumber) {
		...
		for (int i = 0; i < vmsNumber; i++) {
			...
			vms.add(new PowerVm(i, brokerId, Constants.VM_MIPS[vmType], Constants.VM_PES[vmType], Constants.VM_RAM[vmType], Constants.VM_BW, Constants.VM_SIZE, 1, "Xen",
					new CloudletSchedulerDynamicWorkload(Constants.VM_MIPS[vmType], Constants.VM_PES[vmType]), Constants.SCHEDULING_INTERVAL));
		}
		return vms;
	}
	...
	public static List<PowerHost> createHostList(int hostsNumber) {
		...
		for (int i = 0; i < hostsNumber; i++) {
			...
			hostList.add(new PowerHostUtilizationHistory(i, new RamProvisionerSimple(Constants.HOST_RAM[hostType]),
					new BwProvisionerSimple(Constants.HOST_BW), Constants.HOST_STORAGE, peList, new QPowerVmScheduler(peList),
