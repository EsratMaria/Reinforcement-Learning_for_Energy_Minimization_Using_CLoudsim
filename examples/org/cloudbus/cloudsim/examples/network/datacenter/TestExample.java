...
public class TestExample {
	...
	public static void main(String[] args) {
		...
		try {
			...
			broker.setLinkDC(datacenter0); // creating a link between broker and the DC
	...
	private static NetworkDatacenter createDatacenter(String name) {
		...
		for (int i = 0; i < NetworkConstants.EdgeSwitchPort * NetworkConstants.AggSwitchPort
				* NetworkConstants.RootSwitchPort; i++) { // 0, 1 , 2, 3
			// 2. A Machine contains one or more PEs or CPUs/Cores.
			// In this example, it will have only one core.
	...
	static void CreateNetwork(int numhost, NetworkDatacenter dc) {
		...
		for (int i = 0; i < 1; i++) {
			...
			dc.Switchlist.put(edgeswitch[i].getId(), edgeswitch[i]); // Edge switch connected with DC in the path
			// aggswitch[(int)
